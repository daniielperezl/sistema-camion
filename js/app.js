...
function initMap() {
    if (!map) {
        ...
        // Map Click Handler for New Stop
        map.on('click', function(e) {
            ...
            // Remove previous newStopMarker if exists
            if (newStopMarker) {
                map.removeLayer(newStopMarker);
            }

            // Add new marker to visualize
            newStopMarker = L.marker([lat, lng], {
                opacity: 0.7,
    ...
    stops.forEach(stop => {
        ...
        if (stop.lat && stop.lng) {
            ...
            const marker = L.marker([stop.lat, stop.lng], { icon: numberedIcon })
                .addTo(map)
                .bindPopup(`<b>${stop.order_index}. ${stop.name}</b><br><i>${stop.city || ''}</i><br>${stop.address}`);
