...
function calculateAccounts() {
    ...
    // ahora vamos a hacer la siguiente operacion a el valor TOTAL A CONSIGNAR le vamos a restar el valor de TOTAL ENTREGA A QUALA y si ese sera el valor de TOTAL DESCUADRE O TORAL CUADRE
    const descuadre = totalConsignar - totalEntregaQuala;
    const descuadreEl = document.getElementById('total_descuadre');
    descuadreEl.value = descuadre.toFixed(2);

    // si el valor es negativo aparecer en rojo y si es positivo aparecera en verde
    if (descuadre < 0) {
...
async function loadAccountStatus() {
    try {
        ...
        data.forEach(acc => {
            ...
            const tr = document.createElement('tr');
            const descuadreColor = descuadre < 0 ? 'red' : 'green';
            ...
            tr.innerHTML = `
                ...
                <td style="color: ${descuadreColor}; font-weight: bold;">${descuadre.toFixed(2)}</td>
            `;
            tbody.appendChild(tr);
        });

        const balanceEl = document.getElementById('weekly-balance');
        balanceEl.textContent = weeklyBalance.toFixed(2);
        balanceEl.className = weeklyBalance < 0 ? 'balance-negative' : 'balance-positive';
