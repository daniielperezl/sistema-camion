...
function calculateAccounts() {
    ...
    document.getElementById('total_consignar').value = formatCurrency(totalConsignar);

    const totalConsignado = val('total_consignado');
    const totalQr = val('total_qr');

    const totalEntregaQuala = totalConsignado + totalQr;
    document.getElementById('total_entrega_quala').value = formatCurrency(totalEntregaQuala);

    const descuadre = totalEntregaQuala - totalConsignar;
    const descuadreEl = document.getElementById('total_descuadre');

    // Format sign manually + formatted number
    const formattedDescuadre = (descuadre > 0 ? '+' : '') + formatCurrency(descuadre);
...
async function loadAccountStatus() {
    try {
        ...
        data.forEach(acc => {
            ...
            const tConsignar = parseFloat(acc.total_consignar);
            const tEntregado = parseFloat(acc.total_entrega_quala);
            ...
            tr.innerHTML = `
                ...
                <td>${formatCurrency(tConsignar)}</td>
                <td>${formatCurrency(tEntregado)}</td>
        ...
        const formattedBalance = (weeklyBalance > 0 ? '+' : '') + formatCurrency(weeklyBalance);
