...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private fun performSilentLogin(user: String, pass: String) {
        val api = ApiClient.retrofit.create(ApiService::class.java)
        val request = LoginRequest(user, pass)
        api.login(request).enqueue(object : Callback<LoginResponse> {
            override fun onResponse(call: Call<LoginResponse>, response: Response<LoginResponse>) {
                if (!response.isSuccessful || response.body()?.success != true) {
                    // Login failed (password changed?), redirect to Login
                ...
                // If success, the cookie jar is updated automatically.
                // Data fetch will proceed via setupDaySpinner logic.
            }
            override fun onFailure(call: Call<LoginResponse>, t: Throwable) {
                // Network error likely, we can try to fetch anyway or just show toast
